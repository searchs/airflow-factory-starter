...
process_invoices_dag = factory.create(
    "ProcessInvoicesDAG", config.process_invoices
).build()
process_messages_dag = factory.create(
    "ProcessMessagesDAG", config.process_messages
).build()
...
with DAG(
    ...
    subdag_1 = SubDagOperator(
        task_id="task_1", subdag=reconfigure_subdag(dag, task_1_dag, "task_1_name")
    )

    subdag_2 = SubDagOperator(
        task_id="task_2", subdag=reconfigure_subdag(dag, task_2_dag, "task_2_name")
    )
